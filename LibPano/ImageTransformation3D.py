...
if __name__ == '__main__':
    ...
    R = np.array([np.deg2rad(45),0,0],np.float32)
    RR,jacb = cv2.Rodrigues(R)
    P = np.array([0,1,0],np.float32)
    PR = np.dot(RR,P)


    filePath = "LibPano/Images/PanoCar/test.jpg"
    imageTrans = ImageTransformer(filePath,(187,191,3))
    img = imageTrans.rotate_along_axis(0,0,0,0,0,0)
