...
def spliceImg(img,doCrop=False):
    ...
    section = np.int(img.shape[1]/4);
    retVal = []
    for i in range(0,4):
        temp = img[:,section*i:section*(i+1),:]
        if( doCrop ):
...
def main(argv):
    inputfile = 'LibPano/Images//Fisheye/fisheye1.jpg'
    outputdir = 'LibPano/Images//Fisheye/'
    try:
    ...
    #    result = result.edges()
        ...
        #temp.show()
        status = cv2.imwrite("{0}View{1}.png".format(outputdir,1),result)
